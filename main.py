...
with st.container(height=None, border=False, key="metric-container"):
    ...
    svc_fee = (
        svc_fee.str.replace("$", "", regex=False)
        .str.replace(",", "", regex=False)
        .dropna()
    )
...
with st.container(height=None, border=False, key="viz1-container"):
    col1, col2, col3 = st.columns(
        [36, 36, 28], gap="small", border=False, vertical_alignment="center"
    )
    with col3:
    ...
    with col1:
        ...
        data["neighborhood_group"] = (
            data["neighborhood_group"]
            .dropna()
            .str.replace("brookln", "Brooklyn")
            .str.replace("manhatan", "Manhattan")
        )
        # group by according to no. of airbnb hosts per neighborhood group
        loc_group = (
            data.groupby("neighborhood_group")
            .agg({"neighborhood_group": "size"})
            .to_dict()
        )
        loc_group = pd.DataFrame(loc_group).reset_index()
        loc_group = loc_group.rename(
            columns={"index": "neighborhood_group", "neighborhood_group": "no_of_hosts"}
        )
        ...
        loc_group_bar.update_yaxes(
            title="No. of Hosts", showline=True, linecolor="gray"
        )
        loc_group_bar.update_layout(
    ...
    with col2:
        ...
        room_type = room_type.rename(
            columns={"index": "room_type", "room_type": "no_of_hosts"}
        )
        room_type_donut = go.Figure(
            data=[
                go.Pie(
                    values=room_type["no_of_hosts"],
                    labels=room_type["room_type"],
                    hole=0.4,
...
with col1:
    ...
    policy = (
        data.groupby("cancellation_policy")
        .agg({"cancellation_policy": "size"})
        .to_dict()
    )
    policy = pd.DataFrame(policy).reset_index()
    policy = policy.rename(
        columns={"index": "cancellation_policy", "cancellation_policy": "no_of_hosts"}
    )
    policy = go.Figure(
        data=[
            go.Pie(
                values=policy["no_of_hosts"],
                labels=policy["cancellation_policy"],
                hole=0.4,
                textinfo="label+percent",
            )
        ]
...
with col2:
    cons_year = (
        data.groupby("construction_year").agg({"construction_year": "size"}).to_dict()
    )
    cons_year = pd.DataFrame(cons_year).reset_index()
    cons_year = cons_year.rename(
        columns={"index": "construction_year", "construction_year": "no_of_hosts"}
    )
    cons_year_line = px.line(
        cons_year, x="construction_year", y="no_of_hosts", text="no_of_hosts"
    )
    cons_year_line.update_traces(textposition="bottom right")
    cons_year_line.update_xaxes(
        title="Construction Year", showline=True, linecolor="gray"
    )
    cons_year_line.update_yaxes(
        range=[0, 5500], title="No. of Hosts", showline=True, linecolor="gray"
    )
...
# with st.container(height=None, border=False, key="viz3-container"):
...
with col1:
    ...
    top_host = (
        data[["host_name", "number_of_reviews"]]
        .round({"number_of_reviews": 0})
    ...
    top_host = top_host.rename(
        columns={"host_name": "Host Name", "number_of_reviews": "No. of Reviews"}
    )
...
with col2:
    ...
    rating = rating.rename(
        columns={"index": "review_rating", "review_rating": "no_of_hosts"}
    )
    review_ratings = px.bar(
        rating,
        x="no_of_hosts",
        y="review_rating",
        text_auto=True,
        orientation="h",
        labels={"no_of_hosts": "No. of Hosts", "review_rating": "Review Rating"},
    )
...
with col3:
    ...
    top_host2 = data[["host_name", "number_of_reviews", "review_rating"]].sort_values(
        by=["review_rating", "number_of_reviews"], ascending=False
    )
    top_host2 = top_host2.drop_duplicates().head(10).reset_index(drop=True)
    top_host2 = top_host2.rename(
        columns={
            "host_name": "Host Name",
